...
@push('customJs')
    <script>
        ...
        // --- load subcategories and restore old value ---
        function initCategoryDropdown() {
            const oldCat = {!! json_encode(old('category')) !!} || null;
            const oldSub = {!! json_encode(old('sub_category')) !!} || null;

            function load(catId, selectedSub = null) {
                if (!catId) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $.each(response["subCategories"], function(key, item) {
                            const isSelected = (selectedSub && selectedSub == item.id) ? ' selected' :
                                '';
            ...
            // if old category exists (page reloaded after validation fail), load subcategories and select old sub
            if (oldCat) {
                // set the category select to old (in case it wasn't)
                $('#category').val(oldCat);
                load(oldCat, oldSub);
            }

            // when user changes category, load subcategories (no selected)
            $('#category').off('change.initSub').on('change.initSub', function() {
                load($(this).val(), null);
            });
        }
        ...
        // --- Dropzone and image logic (unchanged) ---
